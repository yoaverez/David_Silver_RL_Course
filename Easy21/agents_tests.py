...
def run_and_save_different_lambda(episodes=1000, check_points=[1000]):
    lambdas = np.arange(0, 1.1, 0.1)  # lambda in [0.0, 0.1, 0.2, ..., 0.9, 1.0]
    size = (1000 * 1000, 'M') if episodes >= 1e6 else (1000, 'K')
    log_path = os.path.join("Sarsa_data", "logs", f"{int(episodes / size[0])}{size[1]}_episodes_LAMBDA.txt")
    q_function_path = os.path.join("Sarsa_data", "Q_functions",
...
def plot_mse_with_different_lambdas(lambdas=np.arange(0, 1.1, 0.1), episodes=1000):
    size = (1000 * 1000, 'M') if episodes >= 1e6 else (1000, 'K')
    q_function_path = os.path.join("Sarsa_data", "Q_functions",
                                   f"{int(episodes / size[0])}{size[1]}_episodes_LAMBDA.npy")
    ...
    fig_path = os.path.join("Sarsa_data", "pics",
...
def plot_mse_for_different_number_of_episodes(episodes_list, train=False, log_path=""):
    ...
    if train:
        agent = SarsaControlAgent(log_path=log_path)
